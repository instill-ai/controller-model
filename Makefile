...
dev:							## Run dev container
	@docker compose ls -q | grep -q "instill-model" && true || \
		(echo "Error: Run \"make latest PROFILE=controller ITMODE_ENABLED=true\" in model repository (https://github.com/instill-ai/model) in your local machine first." && exit 1)
